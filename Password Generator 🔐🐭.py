...
def password_generator(L):
    ...
    password = ''.join(random.choices(List, k=L)) #random.choises only will return a list, we need to join it
...
def check_length(L):
    ...
    try:
        ...
        if length in range(16, 51):
            decorate(password_generator(length))
        else:
            print(DECORATION)
